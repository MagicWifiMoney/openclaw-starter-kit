---
name: research
description: "Deep research via Gemini CLI — runs in background sub-agent (cheap, doesn't burn Claude tokens). Use when: multi-source research tasks, competitor analysis, market research, comprehensive topic deep-dives that need 10+ sources. DON'T use when: quick factual lookups (use web_search), checking a single URL (use web_fetch), or time-sensitive questions needing instant answers (Gemini runs async in background)."
